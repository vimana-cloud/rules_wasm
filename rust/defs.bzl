...
def rust_component(name, wit, world = None, deps = None, **kwargs):
    """
    ...
    kwargs["crate_name"] = snake_world
    kwargs["platform"] = Label("@rules_rust//rust/platform:wasip2")
    if "rustc_flags" not in kwargs:
        # Make components as small as possible by default.
        kwargs["rustc_flags"] = ["-C", "opt-level=s"]

    # The library generated by `rust_wit_bindgen`
    # contains the metadata required to by the `wasip2` Rust backend
